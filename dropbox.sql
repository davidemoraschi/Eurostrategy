/* Formatted on 11/18/2011 2:38:45 PM (QP5 v5.163.1008.3004) */
...
CREATE TYPE DROPBOX
       ...
       (oauth_callback VARCHAR2 (1000),
        ...
        MEMBER PROCEDURE get_account_info (p_callback IN VARCHAR2 DEFAULT NULL, p_credentials_in_response OUT CLOB),
        MEMBER PROCEDURE post_html_file (p_filecontent IN CLOB DEFAULT NULL, p_filename IN VARCHAR2 DEFAULT NULL));
...
AS
   ...
   BEGIN
      ...
      p_credentials_in_response := l_clob;
   ...
   BEGIN
      ...
      oauth_api_url := 'https://api-content.dropbox.com/' || SELF.oauth_api_version || '/files/dropbox/Public';

      SELF.oauth_timestamp :=
         TO_CHAR (TRUNC ( (SYSDATE - TO_DATE ('01-01-1970', 'DD-MM-YYYY')) * (86400) - pq_constants.con_num_timestamp_tz_diff));
      SELF.oauth_nonce := SELF.urlencode (SUBSTR (SELF.oauth_timestamp, 6));
      SELF.oauth_base_string :=
         http_method || '&' || SELF.urlencode (oauth_api_url) || '&' || SELF.urlencode ('file=' || urlencode (p_filename))
         || SELF.urlencode (
                  '&oauth_consumer_key='
               ...
               || SELF.oauth_timestamp
               || '&oauth_token='
               || SELF.urlencode (SELF.oauth_access_token)
               || '&oauth_version=1.0');
      ...
      */
      ...
      msg_multipart := msg_multipart || 'Content-Disposition: form-data; name="file"; filename="' || p_filename || '"' || crlf;
      msg_multipart := msg_multipart || 'Content-Type: text/html' || crlf;
      msg_multipart := msg_multipart || crlf;
      msg_multipart := msg_multipart || p_filecontent;
      ...
      SELECT (obj_twitter)
        INTO v_obj_twitter
        FROM objs_twitter
       WHERE ACCOUNT = '382533930';                                   --EUROSTRATEGY: Hay que cambiar esto si cambia la aplicación

      v_obj_twitter.post_status (
         p_status               =>   TO_CHAR (SYSDATE, 'HH24:MI:SS')
                                  || ', Posted file: '
                                  || shortenurl ('http://dl.dropbox.com/u/' || SELF.id || '/' || p_filename),
         p_result_in_response   => p_result_in_response);
   END post_html_file;
END;
/
