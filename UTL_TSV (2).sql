/* Formatted on 28/08/2011 16:10:41 (QP5 v5.139.911.3011) */
CREATE OR REPLACE PACKAGE BODY EUROSTAT.UTL_TSV
AS
   /******************************************************************************
      NAME:       UTL_TSV
      PURPOSE:
   ...
   ******************************************************************************/
   ...
   BEGIN
      ...
      LOOP
         l_dynamic_SQL := l_dynamic_SQL ||'CODE_'|| l_dimension_list (i) || ' VARCHAR2(100), ';
      END LOOP;

      l_dynamic_SQL :=
         l_dynamic_SQL
         || ' CODE_TIME VARCHAR2(50), IND_VALUE NUMBER) NOLOGGING NOMONITORING NOPARALLEL';
      ...
      FOR i IN l_dimension_list.FIRST .. l_dimension_list.LAST
      LOOP
         l_dynamic_SQL := l_dynamic_SQL ||'CODE_'|| l_dimension_list (i) || ', ';
      END LOOP;

      l_dynamic_SQL := l_dynamic_SQL || ' CODE_TIME) ENABLE VALIDATE NOPARALLEL NOLOGGING';
      ...
      LOOP
         l_dynamic_SQL :=
            ...
            || ' (CODE_'
            || l_dimension_list (i)
            || ') ENABLE VALIDATE NOPARALLEL NOLOGGING';

         --log_it (l_dynamic_SQL, l_DEBUG);
         EXECUTE IMMEDIATE l_dynamic_SQL;
      END LOOP;
      
      -- Detect different indicators in the same column
     FOR i IN l_dimension_list.FIRST .. l_dimension_list.LAST
      LOOP
      if instr(l_dimension_list (i), 'indic')>0 THEN
         l_dynamic_SQL := 'SELECT DISTINCT CODE_'||l_dimension_list (i)||' FROM STAGE_'||l_fact_table_name;
         open l_dynamic_CUR for l_dynamic_SQL;
         l_dynamic_IDX := 0;
         loop
         EXIT WHEN l_dynamic_CUR%NOTFOUND; 
         fetch l_dynamic_CUR into l_indicator_list(l_dynamic_IDX);
         l_dynamic_IDX := l_dynamic_IDX + 1;
         end loop;
         close l_dynamic_CUR;
      -- Drops FACT table temp table
          l_dynamic_SQL := 'DROP TABLE FACT_' || l_fact_table_name;
         ...
         l_dynamic_SQL := 'CREATE TABLE FACT_' || l_fact_table_name|| ' (';

          FOR i IN l_dimension_list.FIRST .. l_dimension_list.LAST
          LOOP
             l_dynamic_SQL := l_dynamic_SQL ||'PKEY_'|| l_dimension_list (i) || ' NUMBER, ';
          END LOOP;
         ...
         for l_dynamic_IDX in l_indicator_list.first..l_indicator_list.last
         loop
         l_dynamic_SQL :=
             l_dynamic_SQL
             ||'IND_'||l_indicator_list(l_dynamic_IDX)||' NUMBER, ';
         end loop;
          l_dynamic_SQL :=
             l_dynamic_SQL
             || ' DUMMY VARCHAR2(1)) NOLOGGING NOMONITORING NOPARALLEL';

        DBMS_OUTPUT.put_line (l_dynamic_SQL);
        EXECUTE IMMEDIATE l_dynamic_SQL;
