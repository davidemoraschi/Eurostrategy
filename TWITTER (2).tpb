CREATE OR REPLACE TYPE BODY EUROSTAT.TWITTER
AS
   ...
   BEGIN
      ...
      SELF.oauth_api_request_token_url := 'https://api.twitter.com/oauth/request_token';
      SELF.oauth_api_authorization_url := 'https://api.twitter.com/oauth/authorize';
      SELF.oauth_api_access_token_url := 'https://api.twitter.com/oauth/access_token';
      ...
      BEGIN
         ...
         LOOP
            ...
            THEN
               SELF.oauth_request_token_secret :=
                  SELF.token_extract (p_str => var_http_resp_value, p_pat => 'oauth_token_secret');
               ...
               SELF.oauth_api_authorization_url :=
                  SELF.oauth_api_authorization_url || '?oauth_token=' || SELF.oauth_request_token;
            END IF;
         END LOOP;
      EXCEPTION
   ...
   IS
   BEGIN
      UPDATE objs_twitter c
         SET c.obj_twitter = SELF
       WHERE account = SELF.id;

      IF SQL%ROWCOUNT = 0
      THEN
         INSERT INTO objs_twitter
   ...
   BEGIN
      ...
      SELF.oauth_signature :=
         SELF.
         signature (
            p_oauth_base_string   => SELF.oauth_base_string,
   ...
   BEGIN
      ...
      SELF.oauth_nonce := SELF.urlencode (SUBSTR (oauth_timestamp, 6));
      SELF.oauth_base_string :=
         http_method || '&' || urlencode (oauth_api_url) || '&'
         || urlencode (
                  'include_entities=true&oauth_consumer_key='
               || urlencode (SELF.oauth_consumer_key)
               || '&oauth_nonce='
               || SELF.oauth_nonce
               || '&oauth_signature_method=HMAC-SHA1&oauth_timestamp='
               || SELF.oauth_timestamp
